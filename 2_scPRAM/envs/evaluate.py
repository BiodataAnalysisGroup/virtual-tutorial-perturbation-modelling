...
def evaluate_adata(eval_adata, cell_type, key_dic):
    ...
    sc.pl.umap(eval_adata, color=key_dic['condition_key'], frameon=False,
              title="UMAP of " + cell_type + " by Condition")    
...
def get_wasserstein_distance(eval_adata, case_key='stimulated', pred_key='pred', 
    ...
    dist_list = []
    dist_list_top = []
    pred = eval_adata[(eval_adata.obs[key_dic['condition_key']] == pred_key)].to_df()
    case = eval_adata[(eval_adata.obs[key_dic['condition_key']] == case_key)].to_df()
